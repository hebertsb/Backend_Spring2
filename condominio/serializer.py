...
class PerfilUsuarioSerializer(serializers.ModelSerializer):
    ...
    fecha_registro = serializers.DateTimeField(source='user.date_joined', read_only=True)
    ...
    class Meta:
        model = Usuario
        fields = [
            'id', 'nombre', 'rubro', 'num_viajes', 'rol', 'telefono', 
            'fecha_nacimiento', 'genero', 'documento_identidad', 'pais',
            'email', 'username', 'fecha_registro', 'ultimo_acceso',
            'total_reservas', 'reservas_activas', 'total_gastado',
    ...
    def get_total_gastado(self, obj):
        ...
        total = obj.reservas.filter(estado='PAGADA').aggregate(total=Sum('total'))['total']
...
class SoporteResumenSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Usuario
        fields = ['id', 'nombre', 'tickets_total', 'tickets_abiertos', 'tickets_cerrados', 'ultimo_ticket']
...
class PaqueteServicioSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'dia', 'orden', 'hora_inicio', 'hora_fin', 'notas',
            'punto_encuentro_override', 'punto_encuentro_final', 'servicio'
        ]
    ...
    def get_servicio(self, obj):
        ...
        return {
            ...
            'categoria': obj.servicio.categoria.nombre if obj.servicio.categoria else None,
...
class PaqueteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'precio_base', 'precio_bob', 'cupos_disponibles', 'cupos_ocupados',
            'fecha_inicio', 'fecha_fin', 'estado', 'destacado',
            'imagen_principal', 'punto_salida', 'incluye', 'no_incluye',
            'servicios_incluidos', 'itinerario', 'precios', 'disponibilidad', 
            'campania_info', 'created_at'
        ]
        read_only_fields = ['id', 'created_at']
    
    def get_servicios_incluidos(self, obj):
        """Lista de servicios/destinos incluidos en el paquete"""
        paquete_servicios = PaqueteServicio.objects.filter(paquete=obj).select_related('servicio__categoria')
    ...
    def get_precios(self, obj):
        ...
        return {
            ...
            'descuento_aplicado': precio_original - precio_final if precio_final < precio_original else 0,
            'porcentaje_descuento': ((precio_original - precio_final) / precio_original * 100) if precio_final < precio_original else 0
        }
